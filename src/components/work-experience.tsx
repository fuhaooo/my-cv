...
const ExperienceItem = ({ children }: { children: React.ReactNode }) => (
    <li className="flex items-start mb-1">
        <span className="flex-shrink-0 w-1 h-1 mt-1 mr-1.5 bg-black rounded-full"></span>
...
export default function WorkExperience() {
    ...
    return (
        <div className="bg-white print:bg-white rounded-lg p-5 print:p-3">
            <h2 className="text-sm font-bold text-gray-800 mb-3 border-b border-gray-300 pb-1">工作经历</h2>
            <div className="space-y-3">
                {
                    experienceList.map((experience, index) => (
                        <div key={index}>
                            <div className="flex items-center justify-between mb-1">
                                                            <h3 className="text-sm font-medium text-gray-900">{experience.company} - {experience.position}</h3>
                            <p className="text-xs text-gray-600">{experience.startDate} - {experience.endDate}</p>
                            </div>
