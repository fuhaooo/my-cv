...
const ProjectItem = ({ children }: { children: React.ReactNode }) => (
    <li className="flex items-start mb-1">
        <span className="flex-shrink-0 w-1 h-1 mt-1.5 mr-1.5 bg-black rounded-full"></span>
...
export default function ProjectExperience() {
    ...
    return (
        <div className="bg-white print:bg-white rounded-lg p-5 print:p-3">
            <h2 className="text-sm font-bold text-gray-800 mb-3 border-b border-gray-300 pb-1">项目经历</h2>
            <div className="space-y-3">
                {
                    projectList.map((project, index) => (
                        <div key={index}>
                            ...
                            <p className="text-xs text-gray-600">{project.projectStartTime} - {project.projectEndTime}</p>
                        </div>
                        <p className="text-xs text-gray-700 mb-1">{project.projectDescription}</p>
                        <p className="text-xs text-gray-700 font-medium mb-1">职责描述：</p>
